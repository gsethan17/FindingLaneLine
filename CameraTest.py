...
#    print c.get_video_mode_and_frame_rate_info(m, f)
    ...
    xsize = int(c.get_camera_info()['sensor_resolution'].split('x')[0])
    ysize = int(c.get_camera_info()['sensor_resolution'].split('x')[1])
    rate = p['abs_value']
    t = datetime.datetime.now().strftime("%Y%m%d-%H%M")
    ## saved video parameter setting(15frames per sec, resolution(960, 600))
    fourcc = cv2.VideoWriter_fourcc(*'XVID')
    out = cv2.VideoWriter('./saved_video/'+ t + '.avi', fourcc, rate, (xsize, ysize))
